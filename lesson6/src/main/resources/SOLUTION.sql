...
select * from mark where subject_id = (select id from subject where name='Math');

select s.id, s.name, s.birthday, s.groupnumber from student as s
    INNER JOIN payment as p
    ON s.id = p.student_id WHERE p.type_id = (select id from paymenttype where name = 'WEEKLY');
